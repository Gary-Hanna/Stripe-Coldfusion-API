...
<!--- Start Data related to customer object --->
...
<cfelse>
	...
	<cfset customer_data = updateCustomer(customer_id=customer_id, card=card_token, name=customer_name, email=customer_email, description=customer_description, phone=customer_phone, plan='', coupon='') />
		
	<cfif structKeyExists(customer_data,'error')>
		<cfset errormessage = token_data.error.message />
	<cfelse>
		
	<cfset customer_id = customer_data.id /> 
...
<cffunction name="updateCustomer" access="private">
	<cfargument name="customer_id" default="" required="true">
	...
	<cfhttp url="#stripe_api_url#customers/#Trim(arguments.customer_id)#" method="POST" username="#stripe_api_key#" password="" charset="utf-8" result="customer_result">
